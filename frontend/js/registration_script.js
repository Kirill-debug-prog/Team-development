...
class FormsValidation {
  ...
  validateField(fieldControlElement) {
    ...
    if (
      fieldControlElement.matches("#password_repeated") &&
      passwordRepeatedInput.value.length > 0 &&
      passwordInput.value.length > 0 &&
      passwordRepeatedInput.value !== passwordInput.value
    ) {
  ...
  bindEvents() {
    const requiredControlElements = [
      ...document.querySelector(this.selectors.form).elements,
      ...
      if (!response.ok) {
        ...
        throw new Error(
          json.message === "Username already exists"
            ...
            : "Логин или пароль не соответствуют формату"
        );
      }
...
function makeLoginRequest(login, password) {
  ...
  fetch("http://89.169.3.43/api/auth/login", {
    ...
    .then(async (response) => {
      if (!response.ok) {
        throw new Error(
          "Регистрация прошла успешно, но произошла ошибка при входе. Попробуйте войти самостоятельно"
        );
      }
      return response.json();
    })
