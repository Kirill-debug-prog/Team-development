...
function setUserName() {
  headerElements = document.querySelectorAll(
    ".authorized-menu, .unauthorized-menu, .header-user"
  );
  ...
  document.querySelector("span.last-name").innerHTML = `${localStorage.getItem(
    "lastName"
  )}`;
  document.querySelector("span.first-name").innerHTML = `${localStorage.getItem(
    "firstName"
  )}`;
}
...
async function loadConsultanstCards() {
  try {
    ...
    const container = document.getElementById("mentor-card-container");
    container.innerHTML =
      "<p>Не удалось загрузить карточки консультантов. Попробуйте еще раз позже.</p>";
  }
}
  ...
  cartArticle.innerHTML = `
        ...
        ${
          card.categories.length
            ? `<div class="card-categories">Сферы: <span class="lowercase">${card.categories
        ...
        <div class="card-footer">
            ...
            <a class="show-full-card-button" href="./mentor_card.html?id=${
              card.id
            }">
                Подробнее
            </a>
...
priceInput.forEach((input) => {
  input.addEventListener("change", (e) => {
    ...
    priceProgress.style.left =
      (+priceInput[0].value / priceRangeInput[0].max) * 100 + "%";
    priceRangeInput[1].value = +priceInput[1].value;
    priceProgress.style.right =
      100 - (+priceInput[1].value / priceRangeInput[1].max) * 100 + "%";
  });
});
...
priceRangeInput.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (maxVal - minVal < priceGap) {
      ...
      priceProgress.style.right =
        100 - (maxVal / priceRangeInput[1].max) * 100 + "%";
    }
  });
});
...
experienceInput.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (
      maxExperience - minExperience >= experienceGap &&
      minExperience >= experienceRangeInput[0].min &&
      maxExperience <= experienceRangeInput[1].max
    ) {
      if (e.target.classList.contains("min-experience-input")) {
        experienceRangeInput[0].value = minExperience;
        experienceProgress.style.left =
          (minExperience / experienceRangeInput[0].max) * 100 + "%";
      } else {
        experienceRangeInput[1].value = maxExperience;
        experienceProgress.style.right =
          100 - (maxExperience / experienceRangeInput[1].max) * 100 + "%";
      }
    }
  });
});
...
experienceInput.forEach((input) => {
  input.addEventListener("change", (e) => {
    ...
    experienceProgress.style.left =
      (+experienceInput[0].value / experienceRangeInput[0].max) * 100 + "%";
    experienceRangeInput[1].value = +experienceInput[1].value;
    experienceProgress.style.right =
      100 -
      (+experienceInput[1].value / experienceRangeInput[1].max) * 100 +
      "%";
  });
});
...
experienceRangeInput.forEach((input) => {
  input.addEventListener("input", (e) => {
    ...
    if (maxVal - minVal < experienceGap) {
      ...
      experienceProgress.style.left =
        (minVal / experienceRangeInput[0].max) * 100 + "%";
      experienceProgress.style.right =
        100 - (maxVal / experienceRangeInput[1].max) * 100 + "%";
    }
  });
});
...
async function loadDropdownOptions() {
  try {
    ...
    const optionsContainer = document.getElementById("options");
    optionsContainer.innerHTML =
      "<p>Не удалось загрузить сферы. Попробуйте еще раз позже.</p>";
  }
}
...
function toggleOptionsDisplay() {
  document
    .querySelector(".picked-field-options")
    .classList.toggle("hide-options");
}
...
function appFilters() {
  ...
  const selectedSector = Array.from(
    document.querySelectorAll(".dropdown-options input:checked")
  ...
  if (minExperience > 0)
    queryParams.append("minTotalExperienceYears", minExperience);
...
function updateResults(data) {
  ...
  const maxExperience =
    parseFloat(document.getElementById("max-experience").value) || 100;

  //Фильтруем данные
  const filteredData = data.filter((mentor) => {
    const totalExperience =
      mentor.experiences?.reduce((sum, exp) => sum + exp.durationYears, 0) || 0;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .querySelector(".filter-submit-button")
...
function resetFilters() {
  ...
  document
    .querySelectorAll(".picked-field-options")
    .classList.add("hide-options");
}
